...
//        System.out.println("  병합할 " + branch.name + " 의 부모 브랜치 : " + parent.name);
        ...
        for (int i = 0; i < branch.fileList.length; i++) {
            ...
            for (int j = 0; j < parent.fileList.length; j++) {
                File pFile = parent.fileList[(parent.front + i) % MAX_FILES_SIZE];
                if (pFile.name == null) continue;

                if (cFile.name.equals(pFile.name)) {
                    duplicatedFile = pFile;
                    fileIdx = (parent.front + i) % MAX_FILES_SIZE;
                    break;
                }
            }
...
//
//            if (file.name != null) {
//                File duplicatedFile = duplicatedFileName.get(file.name);
//
//                // 중복된 파일이 있다면
//                if (duplicatedFile != null) {
//
//                    // 부모, 자식이 동일한 이름의 파일을 들고 있을 경우
//                    // 생성 시간이 동일하고 자식 브랜치 파일의 수정시간이 더 최신이면 자식브랜치 반영
//                    // 부모 브랜치 파일 업데이트 진행
//                    if (file.createdTime == duplicatedFile.createdTime && file.editedTime > duplicatedFile.editedTime) {
//                        System.out.println("branch : " + parent.name);
//                        System.out.println("  name : " + duplicatedFile.name);
//                        System.out.println("  data : " + duplicatedFile.data);
//                        System.out.println("  createdTime : " + duplicatedFile.createdTime);
//                        System.out.println("  editedTime : " + duplicatedFile.editedTime);
//                        System.out.println();
//                        System.out.println("branch : " + branch.name);
//                        System.out.println("  name : " + file.name);
//                        System.out.println("  data : " + file.data);
//                        System.out.println("  createdTime : " + file.createdTime);
//                        System.out.println("  editedTime : " + file.editedTime);
//                        System.out.println();
//                        System.out.println(parent.name + " 의 파일을 " + branch.name + " 의 파일로 대체합니다.");
//                        System.out.println("------------------------------------------------");
//
//                        duplicatedFile = file;
//                        duplicatedFile.data = file.data;
//
//                    }
//                }
//                // 중복된 파일이 없다면
