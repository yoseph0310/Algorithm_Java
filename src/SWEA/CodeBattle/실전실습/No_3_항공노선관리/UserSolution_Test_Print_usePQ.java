 ...
 * 시간 최단경로, 비용 최단 경로
 */
public class UserSolution_Test_Print_usePQ {
    ...
    public void init(int n) {
        functionCnt = 0;
        System.out.println(++functionCnt + ". init ");
        N = n;
    ...
    public void add(int mStartAirport, int mEndAirport, int mStartTime, int mTravelTime, int mPrice) {
        System.out.println(++functionCnt + ". add("+mStartAirport+", " + mEndAirport + ", " + mStartTime + ", " + mTravelTime + ", " + mPrice +")");
...
//        print_graph(graphPrice);
    ...
    public int minTravelTime(int mStartAirport, int mEndAirport, int mStartTime) {
        System.out.println(++functionCnt+". minTravelTime(" + mStartAirport + ", " + mEndAirport + ", " + mStartTime + ")");
        ...
        while (!pq.isEmpty()) {
            ...
            if (curStartAirport == mEndAirport) {
                System.out.println("  " + (curDays * 24 + curHours - mStartTime));
    ...
    public int minPrice(int mStartAirport, int mEndAirport) {
        System.out.println(++functionCnt + ". minPrice(" + mStartAirport + ", " + mEndAirport +")");
    ...
    static void print_graph(int[][] graph) {
        System.out.println("  현재 그래프 상태 (0 은 실제로 INF)");
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
