 ...
 *
 */
public class 왕실의_기사대결_Test_Print {
...
//        System.out.println("- NightBoard -");
//        printBoard(nightBoard);
//        printNightStatus();
...
//        System.out.println("----------------------------------------------------------------------");
//        System.out.println(id + " 번 기사 " + dir + " 방향으로 이동");
        // 명령 받은 기사
...
//            System.out.println("이동할 기사 수 : " + movingNightSet.size());
//            System.out.print("( ");
...
//            System.out.println(":: 이동 검사 후 상태 ::");
//            printAllStatus();
        }
//        else System.out.println("** 이동 불가 **");

    }

    static void calDamage() {
//        System.out.println(":: 데미지 계산 ::");
        // 밀린 기사들은 밟은 함정만큼 데미지를 입는다.
        for (Night n : movingNightSet) {
            int damage = 0;
            // 명령 받은 기사는 데미지를 입지 않는다.
            if (n.equals(orderedNight)) {
//                System.out.println("\t명령 받은 기사는 피해를 입지 않습니다.");
                continue;
            }
...
//            System.out.println("\t" + n.id + " 번 기사는 " + damage + " 만큼 피해를 입었음. 현재 hp : " + n.hp);
...
//        System.out.println(night.id + " 번 기사 이동 가능 검사");
        ...
        for (Point p : pointList) {
            ...
            // 경계선 밖이거나 벽이면
            if (isNotBoundary(nx, ny) || chessBoard[nx][ny] == 2) {
//                System.out.println("\t경계선 밖이거나 벽임.");
                return false;
            }
            // 자기 자신이면
            if (nightBoard[nx][ny] == night.id) {
//                System.out.println("\t자기 자신임.");
                continue;
            }
            // 살아있는 다른 기사가 있고 그 기사가 이동이 불가하면
            if (nightBoard[nx][ny] != 0 && nightPool[nightBoard[nx][ny]].isAlive && !checkMove(nightPool[nightBoard[nx][ny]], dir)) {
//                System.out.println("\t살아있는 다른 기사가 있는데 그 기사가 이동이 불가능함");
                return false;
            }
//            System.out.println("\t걸리는 것이 없음");
        }

//        System.out.println("\t이동 불가한 이유가 없음. 이동 가능.");
    ...
    static void printNightStatus() {
        System.out.println("- Night Status -");
        for (int i = 1; i <= N; i++) {
            Night n = nightPool[i];
